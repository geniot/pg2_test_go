...
func (window Window) SaveWindowState() {
	...
	ctx.Config.Set(api.WINDOW_STATE_KEY, strconv.FormatInt(int64(windowState), 10))

	if windowState&sdl.WINDOW_MAXIMIZED <= 0 {
		ctx.Config.Set(api.WINDOW_WIDTH_KEY, strconv.FormatInt(int64(width), 10))
		ctx.Config.Set(api.WINDOW_HEIGHT_KEY, strconv.FormatInt(int64(height), 10))
		ctx.Config.Set(api.WINDOW_XPOS_KEY, strconv.FormatInt(int64(xPos), 10))
		ctx.Config.Set(api.WINDOW_YPOS_KEY, strconv.FormatInt(int64(yPos), 10))
	}
